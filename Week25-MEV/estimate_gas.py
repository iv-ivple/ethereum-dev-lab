...
def main():
    ...
    next_base_fee = int(base_fee * 1.125)  # EIP-1559: max 12.5% increase per block
    print("🔥 EIP-1559 Fee Info:")
    print(f"   Current Base Fee:       {w3.from_wei(base_fee, 'gwei'):.2f} Gwei")
    print(f"   Est. Next Base Fee:     {w3.from_wei(next_base_fee, 'gwei'):.2f} Gwei")
    print(f"   Suggested Priority Fee: {w3.from_wei(w3.eth.max_priority_fee, 'gwei'):.2f} Gwei")
    print()
